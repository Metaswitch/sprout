...
public:
  ...
  // hasn't been on since the most recent call to unquiesce).
  void quiesce();
  ...
  //
  // It is only legal to call this method when the connection tracker is
  // quiescing (there has been a previous call to quiesce without an intervening
  // call to unquiesce).
  void unquiesce();

  // Static method that is registered as a transport state listener so the
  // connection tracker is told when a connection disconnects.
