...
public:
  ...
  void create_binding(RegStore::AoR::Binding& binding, std::string instance_id)
  {
...
TEST_F(GRUUTest, Simple)
{
  ...
  ASSERT_EQ("sip:user@domain.com;gr=hello", binding.gruu(pool));
}
...
TEST_F(GRUUTest, Proper)
{
  ...
  ASSERT_EQ("sip:user@domain.com;gr=urn:uuid:f81d4fae-7dec-11d0-a765-00a0c91e6bf6", binding.gruu(pool));
}
...
TEST_F(GRUUTest, NeedsEscaping)
{
  ...
  ASSERT_EQ("sip:user@domain.com;gr=hel%3blo", binding.gruu(pool));
}
