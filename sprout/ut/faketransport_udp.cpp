...
// Must use UDP, because we don't supply a factory - instead, we rely
// on a singleton instance of the transport.
#define PJSIP_TRANSPORT_FAKE_UDP PJSIP_TRANSPORT_UDP

/**
 * These are the target values for socket send and receive buffer sizes,
 * respectively. They will be applied to FAKE_UDP socket with setsockopt().
 ...
 */
#ifndef PJSIP_FAKE_UDP_SO_SNDBUF_SIZE
/*#   define PJSIP_FAKE_UDP_SO_SNDBUF_SIZE	(24*1024*1024)*/
#   define PJSIP_FAKE_UDP_SO_SNDBUF_SIZE	0
#endif

#ifndef PJSIP_FAKE_UDP_SO_RCVBUF_SIZE
/*#   define PJSIP_FAKE_UDP_SO_RCVBUF_SIZE	(24*1024*1024)*/
#   define PJSIP_FAKE_UDP_SO_RCVBUF_SIZE	0
#endif


/* Struct fake_udp_transport "inherits" struct pjsip_transport */
struct fake_udp_transport
{
 ...
 */
static void init_rdata(struct fake_udp_transport *tp, unsigned rdata_index,
...
/*
 * fake_udp_on_read_complete()
 *
 * This is callback notification from ioqueue that a pending recvfrom()
 * operation has completed.
 */
static void fake_udp_on_read_complete( pj_ioqueue_key_t *key,
				  ...
				  pj_ssize_t bytes_read)
{
    ...
    struct fake_udp_transport *tp = (struct fake_udp_transport*)rdata->tp_info.transport;
    int i;
    pj_status_t status;
	...
	if (bytes_read > MIN_SIZE) {
	    ...
	    /* Since this is FAKE_UDP, the whole buffer is the message. */
	 ...
	 */
	{
	    ...
	    struct fake_udp_transport *rdata_tp ;
	    unsigned rdata_index;

	    rdata_tp = (struct fake_udp_transport*)rdata->tp_info.transport;
	...
	if (status == PJ_SUCCESS) {
		...
		PJSIP_ENDPT_LOG_ERROR((rdata->tp_info.transport->endpt,
				       ...
				       "FAKE_UDP transport stopping! Error"));
		break;
	    }
	}
    }
...
/*
 * fake_udp_on_write_complete()
 *
 * This is callback notification from ioqueue that a pending sendto()
 * operation has completed.
 */
static void fake_udp_on_write_complete( pj_ioqueue_key_t *key,
				   pj_ioqueue_op_key_t *op_key,
				   pj_ssize_t bytes_sent)
{
    struct fake_udp_transport *tp = (struct fake_udp_transport*)
 ...
 */
static pj_status_t fake_udp_send_msg( pjsip_transport *transport,
				 pjsip_tx_data *tdata,
 ...
 * fake_udp_shutdown()
 *
 * Start graceful FAKE_UDP shutdown.
 */
static pj_status_t fake_udp_shutdown(pjsip_transport *transport)
{
...
static void fake_udp_set_pub_name(struct fake_udp_transport *tp,
			     const pjsip_host_port *a_name)
{
    ...
    pj_ansi_snprintf(
	tp->base.info, INFO_LEN, "fake_udp %s [published as %s:%d]",
	local_addr,
...
static void fake_udp_set_socket(struct fake_udp_transport *tp,
			   pj_sock_t sock,
			   const pjsip_host_port *a_name)
{
#if PJSIP_FAKE_UDP_SO_RCVBUF_SIZE || PJSIP_FAKE_UDP_SO_SNDBUF_SIZE
    long sobuf_size;
    pj_status_t status;
#endif

    /* Adjust socket rcvbuf size */
#if PJSIP_FAKE_UDP_SO_RCVBUF_SIZE
    sobuf_size = PJSIP_FAKE_UDP_SO_RCVBUF_SIZE;
...
#if PJSIP_FAKE_UDP_SO_SNDBUF_SIZE
    sobuf_size = PJSIP_FAKE_UDP_SO_SNDBUF_SIZE;
    ...
    fake_udp_set_pub_name(tp, a_name);
}

/* Register socket to ioqueue */
static pj_status_t register_to_ioqueue(struct fake_udp_transport *tp)
{
    ...
    ioqueue_cb.on_read_complete = &fake_udp_on_read_complete;
    ioqueue_cb.on_write_complete = &fake_udp_on_write_complete;
...
static pj_status_t start_async_read(struct fake_udp_transport *tp)
{
	...
	if (status == PJ_SUCCESS) {
	    ...
	    fake_udp_on_read_complete(tp->key, &tp->rdata[i]->tp_info.op_key.op_key,
				 size);
	} else if (status != PJ_EPENDING) {
	    /* Error! */
	    return status;
...
/*
 * pjsip_fake_udp_transport_attach()
 *
 * Attach FAKE_UDP socket and start transport.
 */
				     ...
				     pjsip_transport **p_transport)
{
    ...
    /* Create the FAKE_UDP transport object. */
    tp = PJ_POOL_ZALLOC_T(pool, struct fake_udp_transport);
    ...
    fake_udp_set_socket(tp, sock, a_name);
    ...
    /* Set functions. */
    tp->base.send_msg = &fake_udp_send_msg;
    tp->base.do_shutdown = &fake_udp_shutdown;
    tp->base.destroy = &fake_udp_destroy;
...
on_error:
    fake_udp_destroy((pjsip_transport*)tp);
    return status;
}


PJ_DEF(pj_status_t) pjsip_fake_udp_transport_attach( pjsip_endpoint *endpt,
						pj_sock_t sock,
						const pjsip_host_port *a_name,
						unsigned async_cnt,
						pjsip_transport **p_transport)
{
    return transport_attach(endpt, PJSIP_TRANSPORT_FAKE_UDP, sock, a_name,
			    async_cnt, p_transport);
}

PJ_DEF(pj_status_t) pjsip_fake_udp_transport_attach2( pjsip_endpoint *endpt,
						 pjsip_transport_type_e type,
						 pj_sock_t sock,
						 const pjsip_host_port *a_name,
						 unsigned async_cnt,
...
/*
 * Retrieve the internal socket handle used by the FAKE_UDP transport.
 */
PJ_DEF(pj_sock_t) pjsip_fake_udp_transport_get_socket(pjsip_transport *transport)
{
    struct fake_udp_transport *tp;

    PJ_ASSERT_RETURN(transport != NULL, PJ_INVALID_SOCKET);

    tp = (struct fake_udp_transport*) transport;
 ...
 */
PJ_DEF(pj_status_t) pjsip_fake_udp_transport_pause(pjsip_transport *transport,
					      unsigned option)
{
    struct fake_udp_transport *tp;
    unsigned i;
    ...
    /* Destroy the socket? */
    if (option & PJSIP_FAKE_UDP_TRANSPORT_DESTROY_SOCKET) {
	if (tp->key) {
    ...
    PJ_LOG(4,(tp->base.obj_name, "SIP FAKE_UDP transport paused"));
 ...
 */
PJ_DEF(pj_status_t) pjsip_fake_udp_transport_restart(pjsip_transport *transport,
					        unsigned option,
						pj_sock_t sock,
						...
						const pjsip_host_port *a_name)
{
    ...
    PJ_LOG(4,(tp->base.obj_name,
	      "SIP FAKE_UDP transport restarted, published address is %.*s:%d",
	      (int)tp->base.local_name.host.slen,
	      tp->base.local_name.host.ptr,
	      tp->base.local_name.port));
