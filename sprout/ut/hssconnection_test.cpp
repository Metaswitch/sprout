...
class HssConnectionTest : public BaseTest
{
    ...
    _hss("narcissus", NULL)
  {
    ...
    fakecurl_responses["http://narcissus/impi/privid69/registration-status?impu=pubid44&visited-network=domain&auth-type=auth-type"] = "{\"result-code\": 2001, \"mandatory-capabilities\": [1, 2, 3], \"optional-capabilities\": []}";
    fakecurl_responses["http://narcissus/impu/pubid44/location"] = "{\"result-code\": 2001, \"scscf\": \"server-name\"}";
    fakecurl_responses["http://narcissus/impu/pubid44/location?auth-type=auth-type"] = "{\"result-code\": 2001, \"mandatory-capabilities\": [], \"optional-capabilities\": []}";
    fakecurl_responses["http://narcissus/impu/pubid44/location?originating=true&auth-type=auth-type"] = "{\"result-code\": 2001, \"mandatory-capabilities\": [1, 2, 3], \"optional-capabilities\": []}";
 }
...
{
  Json::Value* actual = _hss.get_user_auth_request("privid69", "pubid44", "", "", 0);
  ASSERT_TRUE(actual != NULL);
...
{
  Json::Value* actual = _hss.get_user_auth_request("privid69", "pubid44", "domain", "auth-type", 0);
  ASSERT_TRUE(actual != NULL);
...
{
  Json::Value* actual = _hss.get_location_information_request("pubid44", "", "", 0);
  ASSERT_TRUE(actual != NULL);
  EXPECT_EQ("server-name", actual->get("scscf", "").asString());
  delete actual;
}
