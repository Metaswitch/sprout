...
void PJUtils::get_impi_and_impu(pjsip_rx_data* rdata, std::string& impi_out, std::string& impu_out)
{
  ...
  {
    // Check to see if the request has an explicit IMPI in the Authorization header
    pjsip_authorization_hdr* auth_hdr = (pjsip_authorization_hdr*)
  ...
  else
  {
    ...
    pjsip_uri* impu_uri = orig_served_user(rdata->msg_info.msg);
    ...
    else
    {
      ...
      impi_out = PJUtils::default_private_id_from_uri(impu_uri);
      TRC_DEBUG("Private identity defaulted from public identity = %s", impi_out.c_str());
    }
  }
}
