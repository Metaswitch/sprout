...
void send_register_to_as(pjsip_rx_data* received_register, pjsip_tx_data* ok_response, AsInvocation* as, int expires)
{
  ...
  char trusted_port[6] = "";
  snprintf(trusted_port, 6, "%d", stack_data.trusted_port);
  ...
  if (received_register && ok_response) {
    ...
    pjsip_hdr* original_pani_hdr = pjsip_msg_find_hdr_by_name(received_register->msg_info.msg, &pani_pjstr, NULL);
  ...
  printf("Outgoing REGISTER request to AS: %s\n", for_printing);
