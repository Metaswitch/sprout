...
pj_status_t SproutletProxy::UASTsx::init(pjsip_rx_data* rdata)
{
  ...
  if (rdata->msg_info.msg->line.req.method.id != PJSIP_ACK_METHOD)
  {
    ...
    // send these off the box, so set a flag to absorb ACKs that are received
                                                    ...
                                                    pjsip_tx_data *rsp)
{
  ...
  if (i != _umap.end())
  {
    ...
    upstream_sproutlet->rx_response(rsp, upstream_fork, true);
        ...
        (mf_hdr->ivalue <= 0))
    {
      ...
      if (req.req->msg->line.req.method.id != PJSIP_ACK_METHOD)
      {
        ...
        if (status == PJ_SUCCESS)
        {
          ...
          req.upstream.first->rx_response(rsp, req.upstream.second, false);
        }
      }
      else
      {
                                         ...
                                         pjsip_tx_data* rsp)
{
  ...
  else
  {
    ...
    if (i != _umap.end())
    {
      ...
      // We do not treat 200 OK to CANCEL as a final response.
      if ((rsp->msg->line.status.code >= PJSIP_SC_OK) &&
          (rsp->msg->line.status.code < 300) &&
          (PJSIP_MSG_CSEQ_HDR(rsp->msg)->method.id != PJSIP_CANCEL_METHOD))
      {
        // Final 2xx response, so break the linkage between the Sproutlets.
        // For 3xx-6xx responses, the linkage between the sproutlets will be
        // broken once the ACK to the response has been sent.
        _dmap_sproutlet.erase(i->second);
        _umap.erase(i);
      }
      upstream->rx_response(rsp, fork_id, false);
    }
    else
    {
...
void SproutletWrapper::rx_response(pjsip_tx_data* rsp, int fork_id, bool client_rsp)
{
  ...
  // If this is a negative response, send an immediate ACK response. We don't
