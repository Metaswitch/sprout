...
pj_status_t SproutletProxy::UASTsx::init(pjsip_rx_data* rdata)
{
  ...
  if (status == PJ_SUCCESS)
  {
    ...
    SproutletTsx* sproutlet_tsx = get_sproutlet_tsx(sproutlet,
                                                    _req,
                                                    alias);
...
void SproutletProxy::UASTsx::process_cancel_request(pjsip_rx_data* rdata)
{
  ...
  if (_root != NULL)
  {
    // Create a CANCEL from the original request and pass it to the Sproutlet
...
SproutletTsx* SproutletProxy::UASTsx::get_sproutlet_tsx(Sproutlet*& sproutlet,
                                                        pjsip_tx_data* req,
                                                        std::string& alias)
{
  ...
  while (sproutlet != NULL)
  {
    ...
    sproutlet = target_sproutlet(req->msg,
                                 0,
                                 alias,
                                 trail());
  }
  ...
  _trail_id(trail_id)
{
  ...
  if (_sproutlet_tsx == NULL)
  {
    ...
    // handle the request.
    _sproutlet_tsx = new SproutletTsx();
  }

  // Initialize the Tsx
  _sproutlet_tsx->init_tsx(this, req->msg);
