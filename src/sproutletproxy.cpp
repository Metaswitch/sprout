                                         ...
                                         pjsip_tx_data* rsp)
{
  ...
  else
  {
    ...
    if (i != _umap.end())
    {
      ...
      // We only treat this like a final response if the code is >= 200 and
      // this is not a 200 OK to a CANCEL.
      if ((rsp->msg->line.status.code >= PJSIP_SC_OK) &&
          (((pjsip_cseq_hdr*)pjsip_msg_find_hdr(rsp->msg, PJSIP_H_CSEQ, NULL))->method.id != PJSIP_CANCEL_METHOD))
      {
...
void SproutletWrapper::rx_response(pjsip_tx_data* rsp, int fork_id, bool client_rsp)
{
  ...
  if (((pjsip_cseq_hdr*)pjsip_msg_find_hdr(rsp->msg, PJSIP_H_CSEQ, NULL))->method.id == PJSIP_CANCEL_METHOD)
  {
    pjsip_tx_data_dec_ref(rsp);
  }
  else
