...
{
  printf("thing recieving error = %s\n", _id.c_str());
  printf("error = %s\n", fork_error_to_str(fork_error));
  printf("fork id = %d\n", fork_id);
  printf("tsx_state_str = %s\n", pjsip_tsx_state_str(_forks[fork_id].state.tsx_state));
  TRC_VERBOSE("%s received error %s on fork %d, state = %s",
              _id.c_str(), fork_error_to_str(fork_error),
              fork_id, pjsip_tsx_state_str(_forks[fork_id].state.tsx_state));
  printf("don't make it here\n");
...
void SproutletWrapper::process_actions(bool complete_after_actions)
{
  ...
  // So, to get the code to send the response I need to take fork off
  // _pending_responses when I cancel it.
  if ((!_complete) &&
      (_best_rsp != NULL) &&
