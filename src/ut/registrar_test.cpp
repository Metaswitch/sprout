...
TEST_F(RegistrarTest, BadSchemeFetchBindings)
{
  Message msg;
  msg._scheme = "sips";
  msg._contact = "";
  ...
  EXPECT_EQ(0,((SNMP::FakeSuccessFailCountTable*)SNMP::FAKE_REGISTRATION_STATS_TABLES.init_reg_tbl)->_failures);
}
...
TEST_F(RegistrarTest, AssociatedUrisNotFoundFetchBindings)
{
  Message msg;
  msg._user = "6505550232";
  msg._contact = "";
  ...
  EXPECT_EQ(403, out->line.status.code);
  EXPECT_EQ("Forbidden", str_pj(out->line.status.reason));
  EXPECT_EQ(0,((SNMP::FakeSuccessFailCountTable*)SNMP::FAKE_REGISTRATION_STATS_TABLES.de_reg_tbl)->_attempts);
  EXPECT_EQ(0,((SNMP::FakeSuccessFailCountTable*)SNMP::FAKE_REGISTRATION_STATS_TABLES.de_reg_tbl)->_failures);
}
...
TEST_F(RegistrarTestMockStore, SubscriberDataManagerGetsFailFetchBindings)
{
  EXPECT_CALL(*_local_data_store, get_data(_, _, _, _, _))
    .WillOnce(Return(Store::ERROR));
